...
import { INSTA_ICONS, INSTA_PRICES } from "./constants";
...
const InstagramOption = ({ selected, onSelect }) => (
  <div className="bg-white p-5 sm:p-6 rounded-3xl border border-gray-100 shadow-sm">
    ...
    <div className="flex items-center gap-4 mb-8">
      ...
      <div>
        <h4 className="text-xl font-black text-gray-800">ترويج انستقرام</h4>
        <div className="flex items-center gap-2 mt-1">
          <span className="inline-block w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
          <p className="text-sm text-gray-500 font-medium">نشر عبر حسابنا (57k+ متابع)</p>
        </div>
      </div>
    </div>

    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
      {INSTA_OPTS.map(({ key, label }) => {
        ...
        return (
          ...
          >
            ...
            <div className={`text-3xl transition-transform duration-300 ${isSelected ? "scale-110" : "group-hover:scale-110 grayscale-[0.5] group-hover:grayscale-0"}`}>
              {INSTA_ICONS[key]}
            </div>

          
            <div className="text-center">
              <span className={`block font-bold text-sm mb-1 ${isSelected ? "text-orange-700" : "text-gray-600"}`}>
                {label}
              </span>
              <div className="flex items-baseline justify-center gap-0.5">
                <span className={`font-sans text-lg font-black ${isSelected ? "text-orange-600" : "text-gray-900"}`}>
                  {INSTA_PRICES[key]}
                </span>
                <span className="text-[10px] font-bold text-gray-400">د.ك</span>
              </div>
            </div>
          </div>
        );
